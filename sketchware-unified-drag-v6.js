...
/*
 * Unified Logic Drag v6
 * One pointer router for the Sketchware-style editor.
 * - palette/statement/reporter long-press drag
 * - native form controls keep normal click/select behavior
 * - existing statement drag ALWAYS exposes Delete/Duplicate/Collection
 * - action-strip visibility is driven by body.sw6-existing-drag
 */
...
function palette(){return document.getElementById('swPalette');}
...
function insideLogic(el){var o=overlay();return !!(o&&el&&o.contains(el));}
function esc(v){return String(v==null?'':v).replace(/[&<>"']/g,function(c){return{'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c];});}
function icon(n){return'<span class="bw-google-icon">'+n+'</span>';}
function nativeControl(t){return !!(t&&t.closest&&t.closest('select,input,textarea,option,[contenteditable="true"]'));}

function sourceInfo(target){
  if(!target||!target.closest||!insideLogic(target)||nativeControl(target))return null;
  ...
  var paletteItem=target.closest('.sw-palette-item[data-sw-type]');
  if(paletteItem)return{kind:'statement-template',el:paletteItem,type:paletteItem.dataset.swType,source:'base'};

  /* Fallback sockets/menu-looking spans are part of the statement drag surface.
     Only real form controls above are excluded. */
  var block=target.closest('#swFlow .sw-block[data-block-id]');
...
function ghostMarkup(info,block){
  ...
  var name='block';
  try{name=(window.BLOCK_META&&BLOCK_META[block.type]&&BLOCK_META[block.type].name)||block.type||'block';}catch(_){}
...
function ensureActions(){
  var found=document.getElementById('sw5DragActions');
  if(found!==actionBar)actionBar=found||null;
  if(!actionBar){
    actionBar=document.createElement('div');
    actionBar.id='sw5DragActions';actionBar.className='sw5-drag-actions';
    actionBar.innerHTML='<div class="sw5-drag-action" data-sw5-drag-action="delete">'+icon('backspace')+'<span>Delete</span></div>'+
      '<div class="sw5-drag-action" data-sw5-drag-action="duplicate">'+icon('content_copy')+'<span>Duplicate</span></div>'+
      '<div class="sw5-drag-action" data-sw5-drag-action="collection">'+icon('bookmark_border')+'<span>Collection</span></div>';
  }
  if(actionBar.parentNode!==document.body)document.body.appendChild(actionBar);
  return actionBar;
}
function positionActions(){
  var b=ensureActions(),p=palette(),bottom=0;
  if(p){var r=p.getBoundingClientRect();if(r.height>2&&r.bottom>0&&r.top<window.innerHeight)bottom=Math.max(0,window.innerHeight-r.top);}
  b.style.setProperty('--sw6-action-bottom',bottom+'px');
}
function showActions(){
  positionActions();var b=ensureActions();
  b.querySelectorAll('.hot').forEach(function(x){x.classList.remove('hot');});
  b.classList.add('show');b.hidden=false;
  document.body.classList.add('sw6-existing-drag');
}
function hideActions(){
  document.body.classList.remove('sw6-existing-drag');
  if(actionBar){actionBar.classList.remove('show');actionBar.querySelectorAll('.hot').forEach(function(x){x.classList.remove('hot');});}
  actionTarget='';
}
function actionAt(x,y){
  if(!isStatementExisting(session&&session.info))return'';
  positionActions();var b=ensureActions(),found='';
  b.querySelectorAll('[data-sw5-drag-action]').forEach(function(el){var r=el.getBoundingClientRect();if(x>=r.left&&x<=r.right&&y>=r.top&&y<=r.bottom)found=el.dataset.sw5DragAction;});
  return found;
}
function highlightAction(name){
  if(!actionBar)return;actionBar.querySelectorAll('[data-sw5-drag-action]').forEach(function(el){el.classList.toggle('hot',el.dataset.sw5DragAction===name);});
}

function clearTargets(){if(hot){hot.classList.remove('hot');hot=null;}if(bad){bad.classList.remove('bad');bad=null;}highlightAction('');}
function lockMobileScroll(){
  if(!mobile()||!session)return;
  var s=stage();if(s){s.scrollTop=session.stageTop;s.scrollLeft=session.stageLeft;}
}
...
function markBadSocket(x,y,block){
...
function update(x,y){
  if(!session)return;
  moveGhost(x,y);clearTargets();lockMobileScroll();
  if(isStatementExisting(session.info))showActions();
  actionTarget=actionAt(x,y);
  if(actionTarget){highlightAction(actionTarget);session.connection.target=null;return;}
  var eng=engine(),target=eng&&eng.update?eng.update(session.connection,x,y):null;
  if(target&&target.element){hot=target.element;hot.classList.add('hot');}else markBadSocket(x,y,session.block);
  autoScroll(x,y);lockMobileScroll();
}
...
function deleteExisting(s){
  ...
  if(isReporterInfo(s.info)){
    var t=typed();if(!t||!t.detach)return false;var removed=t.detach(s.block.id);if(!removed)return false;
  }else{
    var g=graph();if(!g||!g.detach)return false;var d=g.detach(s.block.id,{silent:true});if(!d)return false;if(g.refresh)g.refresh();
  }
...
function duplicateExisting(s){
  if(!s||!isStatementExisting(s.info)||typeof findBlock!=='function')return false;
  var f=findBlock(s.block.id);if(!f)return false;f.list.splice(f.index+1,0,cloneBlock(f.block));saveAndRender();if(typeof toast==='function')toast('Bloco duplicado');return true;
}
function collectExisting(s){
  if(!s||!isStatementExisting(s.info))return false;
  try{var arr=JSON.parse(localStorage.getItem('brotware_block_collection_v1')||'[]');if(!Array.isArray(arr))arr=[];arr.push({savedAt:Date.now(),block:JSON.parse(JSON.stringify(s.block))});if(arr.length>50)arr=arr.slice(arr.length-50);localStorage.setItem('brotware_block_collection_v1',JSON.stringify(arr));if(typeof toast==='function')toast('Bloco adicionado à Collection');return true;}catch(e){console.error(e);return false;}
}
...
function cleanupDrag(){
  ...
  clearTargets();hideGhost();hideActions();
  var o=overlay();if(o)o.classList.remove('sw-dragging','sw5-dragging');
...
function finish(cancelled){
  ...
  suppressClickUntil=Date.now()+850;var o=overlay();if(o)o.dataset.suppressClickUntil=String(suppressClickUntil);
  ...
  var eng=engine();if(!s.connection||!s.connection.target||!eng||!eng.commit||!eng.commit(s.connection)){
...
function touchMove(e){if(session&&mobile())e.preventDefault();}

function begin(info,data){
  var eng=engine(),block=blockFromInfo(info);if(!eng||!block)return false;
  var connection=eng.begin(block);if(!connection)return false;
  var s=stage();session={info:info,block:block,connection:connection,pointerId:data.pointerId,stageTop:s?s.scrollTop:0,stageLeft:s?s.scrollLeft:0};
  sourceEl=info.el;if(sourceEl)sourceEl.classList.add('sw5-drag-source');
  var o=overlay();if(o)o.classList.add('sw-dragging','sw5-dragging');document.body.classList.add('sw5-logic-dragging');
  if(isStatementExisting(info))showActions();else hideActions();
  ...
  update(data.clientX,data.clientY);return true;
}

function cancelPending(){
  if(!pending)return;clearTimeout(pending.timer);window.removeEventListener('pointermove',pendingMove,true);window.removeEventListener('pointerup',pendingUp,true);window.removeEventListener('pointercancel',pendingCancel,true);pending=null;
}
function pendingMove(e){
  if(!pending||e.pointerId!==pending.pointerId)return;
  var dx=e.clientX-pending.x,dy=e.clientY-pending.y;
  if(Math.hypot(dx,dy)>MOVE_CANCEL)cancelPending();
}
...
function pointerDown(e){
  if(session||pending)return;if(e.pointerType==='mouse'&&e.button!==0)return;
  var info=sourceInfo(e.target);if(!info)return;
  /* Stop old flow/palette pointer handlers, but keep other window listeners alive
     (notably the dedicated palette scroll helper). */
  e.stopPropagation();
  pending={info:info,pointerId:e.pointerId,x:e.clientX,y:e.clientY,data:{pointerId:e.pointerId,clientX:e.clientX,clientY:e.clientY}};
  pending.timer=setTimeout(function(){if(!pending)return;var p=pending;cancelPending();begin(p.info,p.data);},LONG_PRESS);
  window.addEventListener('pointermove',pendingMove,true);window.addEventListener('pointerup',pendingUp,true);window.addEventListener('pointercancel',pendingCancel,true);
}
...
function emergencyCleanup(){if(session)finish(true);else cancelPending();document.body.classList.remove('sw6-existing-drag','sw5-logic-dragging');hideActions();}

ensureActions();hideActions();
window.addEventListener('pointerdown',pointerDown,true);
window.addEventListener('click',clickCapture,true);
document.addEventListener('touchmove',touchMove,{capture:true,passive:false});
window.addEventListener('resize',function(){if(session&&isStatementExisting(session.info))positionActions();});
window.addEventListener('blur',emergencyCleanup);
document.addEventListener('visibilitychange',function(){if(document.hidden)emergencyCleanup();});

window.BrotwareUnifiedLogicDrag={version:6,active:function(){return!!session;},cancel:emergencyCleanup,refreshActions:function(){if(session&&isStatementExisting(session.info))showActions();}};
})();
