...
function firstRef(){var c=currentRef();if(c)return c;var a=list(),r=a.find(function(x){return x.tag!=='BODY';})||a[0];return r?r.ref:'';}
function firstInputRef(){var c=currentRef(),cr=c&&rowForRef(c);if(cr&&/^(INPUT|TEXTAREA|SELECT)$/.test(cr.tag))return c;var r=list().find(function(x){return /^(INPUT|TEXTAREA|SELECT)$/.test(x.tag);});return r?r.ref:firstRef();}
