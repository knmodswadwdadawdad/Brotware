...
function setDevice(d){
  ...
  if(deviceBtn)deviceBtn.textContent=device.charAt(0).toUpperCase()+device.slice(1);
}
...
function runToggle(){
  ...
  syncRun();
  toastMsg(editing?'Modo de interação':'Modo de edição');
}
function syncRun(){
  if(!runBtn||!window.BrotwareHybridEditor)return;
  var editing=BrotwareHybridEditor.isEdit&&BrotwareHybridEditor.isEdit();
  runBtn.classList.toggle('active',!editing);
  runBtn.title=editing?'Executar / interagir':'Voltar para edição';
  runBtn.innerHTML=editing?'▶':'✎';
}
...
function buildTop(){
  ...
  if(title){title.classList.add('bw-import-native-title');var st=hs();var strong=title.querySelector('strong'),small=title.querySelector('small');if(strong)strong.textContent=basename(st.entry);if(small)small.textContent='Interface';}
  var back=document.getElementById('bwExternalBack');if(back){back.classList.add('bw-import-native-back');back.textContent='←';}
  if(topTools)return;
...
function wireDock(){
  ...
  var btns=dock.querySelectorAll('button');if(btns.length>=4){
    btns[0].dataset.bwNativeNav='add';btns[0].innerHTML='<span>＋</span><b>Adicionar</b>';
    btns[1].innerHTML='<span>☷</span><b>Camadas</b>';
    btns[2].innerHTML='<span>⚡</span><b>Lógica</b>';
    btns[3].dataset.bwNativeNav='config';btns[3].innerHTML='<span>⚙</span><b>Config.</b>';
  }
...
function syncIssueCount(){var b=document.getElementById('bwImportIssues'),box=document.getElementById('bwExternalErrors');if(!b||!box)return;var n=box.children.length;b.dataset.count=String(n);b.classList.toggle('has-issues',n>0);b.innerHTML='!'+(n?'<i>'+n+'</i>':'');}
...
function install(){if(installed)return;if(!window.BrotwareExternalHybrid||!window.BrotwareHybridEditor){setTimeout(install,120);return;}installed=true;window.addEventListener('brotware:external-open',onOpen);window.addEventListener('brotware:external-ready',refresh);window.addEventListener('brotware:external-selection',refresh);window.addEventListener('brotware:external-snapshot',refresh);window.addEventListener('brotware:external-close',onClose);new MutationObserver(function(){if(active())refresh();}).observe(document.body,{childList:true,subtree:true});setInterval(function(){if(active())refresh();},900);}
