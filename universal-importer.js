...
function isExternalManifest(manifest){return !!(manifest&&/^(?:static|hybrid|framework)$/i.test(String(manifest.mode||'')));}
function isNativeManifest(manifest){
  if(!manifest||!BrotwareProjectFormat.isBrotwareJson(manifest)||isExternalManifest(manifest))return false;
  return !!(!manifest.mode||manifest.mode==='native'||Array.isArray(manifest.pages)||(manifest.data&&Array.isArray(manifest.data.pages)));
}
...
async function importJsonFile(file){
  ...
  if(obj.files){var files={};Object.keys(obj.files).forEach(function(path){var f=obj.files[path]||{},content=f.content||'';if(f.encoding==='base64'){var bin=atob(content),u=new Uint8Array(bin.length);for(var i=0;i<bin.length;i++)u[i]=bin.charCodeAt(i);files[path]=BrotwareVFS.fromBytes(path,u,f.mime);}else files[path]={path:path,mime:f.mime||BrotwareVFS.mime(path),kind:'text',content:String(content),size:new TextEncoder().encode(String(content)).length,modifiedAt:Date.now()};});var rec=await BrotwareProjectFormat.createExternal({projectName:obj.projectName,files:files,entry:obj.entry,mode:obj.mode,framework:obj.framework,requiresBuild:obj.requiresBuild,bindings:obj.bindings,overrides:obj.overrides,config:obj.config,projectData:obj.projectData||null,domPatches:obj.domPatches||[],source:'json'});return{record:rec,kind:'external',message:'Projeto externo Brotware restaurado do JSON.'};}
...
window.BrotwareUniversalImporter={open:openPicker,importFile:importFile,importZip:importZip,importHtml:importHtmlFile,importJson:importJsonFile,isLegacyBundle:isLegacyBrotwareBundle,legacyBindings:legacyBindings,isNativeManifest:isNativeManifest,isExternalManifest:isExternalManifest};
