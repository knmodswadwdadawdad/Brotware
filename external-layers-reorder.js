...
function pointerMove(e){if(!drag||e.pointerId!==drag.pointerId)return;var dx=e.clientX-drag.x,dy=e.clientY-drag.y;if(!drag.started){if(drag.touch){if(Math.hypot(dx,dy)>9){finish(e.clientX,e.clientY,true);return;}}else if(Math.hypot(dx,dy)>4)startNow(e.clientX,e.clientY);}if(drag&&drag.started){e.preventDefault();moveGhost(e.clientX,e.clientY);updateDrop(e.clientX,e.clientY);}}
...
function pointerDown(e){
  ...
  drag={pointerId:e.pointerId,x:e.clientX,y:e.clientY,source:src,sourceRow:row,started:false,touch:e.pointerType!=='mouse',timer:null};if(drag.touch)drag.timer=setTimeout(function(){if(drag)startNow(drag.x,drag.y);},280);window.addEventListener('pointermove',pointerMove,true);window.addEventListener('pointerup',pointerUp,true);window.addEventListener('pointercancel',pointerCancel,true);
}
