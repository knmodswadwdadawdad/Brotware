...
var panel=null,layers=null,layersBody=null,current=null,currentTree=null,collapsed={},installed=false;
var EDIT=[
...
function nodeLabel(n){return n.tag+(n.id?'#'+n.id:'')+(n.classes?'.'+String(n.classes).trim().split(/\s+/).slice(0,2).join('.'):'');}
...
function install(){
  if(installed)return;if(!window.BrotwareExternalPreview){setTimeout(install,120);return;}build();if(!panel){setTimeout(install,120);return;}installed=true;
